...
var Server = exports = module.exports = function (opts) {
  ...
  app.exposeModule(__dirname + '/events', 'events')
  app.exposeModule(__dirname + '/../node_modules/eventstack/lib/eventstack', 'eventstack')
  app.exposeModule(__dirname + '/browser/simpl', 'simpl')

  if (app.get) {
    ;[ 'date', 'events', 'json', 'log', 'rpc', 'uid' ]
    .forEach(function (m) {
      app.exposeModule(__dirname + '/middleware/' + m, 'middleware/' + m)
    })
